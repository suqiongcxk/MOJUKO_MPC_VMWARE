...
KalmanFilterEstimate::KalmanFilterEstimate(const rclcpp::Node::SharedPtr& node, ocs2::PinocchioInterface pinocchioInterface, ocs2::CentroidalModelInfo info,
  ...
  c_.block(0, 6, dimContacts_, dimContacts_) = -ocs2::matrix_t::Identity(dimContacts_, dimContacts_);
...
ocs2::vector_t KalmanFilterEstimate::update(const ocs2::scalar_t& time, const ocs2::scalar_t& period) {
  ...
  q_.block(6, 6, dimContacts_, dimContacts_) = dt * ocs2::matrix_t::Identity(dimContacts_, dimContacts_);
  ...
  vPino.setZero();
  vPino.segment<3>(3) = ocs2::getEulerAnglesZyxDerivativesFromGlobalAngularVelocity<ocs2::scalar_t>(
      qPino.segment<3>(3),
  ...
  const auto eePos = eeKinematics_->getPosition(ocs2::vector_t());
  const auto eeVel = eeKinematics_->getVelocity(ocs2::vector_t(), ocs2::vector_t());

  ocs2::matrix_t q = ocs2::matrix_t::Identity(numState_, numState_);
  ...
  ocs2::matrix_t r = ocs2::matrix_t::Identity(numObserve_, numObserve_);
  ...
  for (int i = 0; i < numContacts_; i++) {
    ...
    ocs2::scalar_t high_suspect_number(100);
  ...
  vector3_t accel = ocs2::getRotationMatrixFromZyxEulerAngles(quatToZyx(quat_)) * linearAccelLocal_ + g;

  ocs2::vector_t y(numObserve_);
  y << ps_, vs_, feetHeights_;
  xHat_ = a_ * xHat_ + b_ * accel;
  ocs2::matrix_t at = a_.transpose();
  ocs2::matrix_t pm = a_ * p_ * at + q;
  ocs2::matrix_t cT = c_.transpose();
  ocs2::matrix_t yModel = c_ * xHat_;
  ocs2::matrix_t ey = y - yModel;
  ocs2::matrix_t s = c_ * pm * cT + r;

  ocs2::vector_t sEy = s.lu().solve(ey);
  xHat_ += pm * cT * sEy;

  ocs2::matrix_t sC = s.lu().solve(c_);
  p_ = (ocs2::matrix_t::Identity(numState_, numState_) - pm * cT * sC) * pm;

  ocs2::matrix_t pt = p_.transpose();
  p_ = (p_ + pt) / 2.0;
...
nav_msgs::msg::Odometry KalmanFilterEstimate::getOdomMsg() {
  ...
  ocs2::vector_t twist = ocs2::getRotationMatrixFromZyxEulerAngles(quatToZyx(quat_)).transpose() * xHat_.segment<3>(3);
...
void KalmanFilterEstimate::loadSettings(const std::string& taskFile, bool verbose) {
  ...
  ocs2::loadData::loadPtreeValue(pt, footRadius_, prefix + "footRadius", verbose);
  ocs2::loadData::loadPtreeValue(pt, imuProcessNoisePosition_, prefix + "imuProcessNoisePosition", verbose);
  ocs2::loadData::loadPtreeValue(pt, imuProcessNoiseVelocity_, prefix + "imuProcessNoiseVelocity", verbose);
  ocs2::loadData::loadPtreeValue(pt, footProcessNoisePosition_, prefix + "footProcessNoisePosition", verbose);
  ocs2::loadData::loadPtreeValue(pt, footSensorNoisePosition_, prefix + "footSensorNoisePosition", verbose);
